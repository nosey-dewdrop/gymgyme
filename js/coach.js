...
// ── LOOP 02 OVERLAY: sabahki İNCE ZARİF vişne köşe-kutu overlay'i (Damla'nın
// beğendiği "real CV motoru" hissi) + renk-kod. Her vücut parçası = köşe-işaretli
// ince tespit kutusu + etiket. İzlenen parça (hareketin eklemi) form iyiyse NANE,
// düzeltme varsa VİŞNE pulse. Renk kaynağı GERÇEK: motorun formCue'su. Ağ/dolgu
// YOK — video net kalır, üstünde takip kutuları oynar. Yüz mesh YOK. ──
const MINT = [58, 214, 160];   // doğru / nane (rgb)
const CHERRY = [166, 27, 66];  // nötr + düzelt / vişne (site paleti)
// MediaPipe pose landmark indisleri.
const LX = { LSHO: 11, RSHO: 12, LELB: 13, RELB: 14, LWRI: 15, RWRI: 16,
             LHIP: 23, RHIP: 24, LKNE: 25, RKNE: 26, LANK: 27, RANK: 28 };
// her vücut parçası: etiket + landmark noktaları + hangi eklem adına ait
// (izlenen eklem eşleşince o kutu renk-kod taşır).
const BODY_PARTS = [
  { label: "torso",     pts: [LX.LSHO, LX.RSHO, LX.LHIP, LX.RHIP], joints: ["shoulder", "hip", "back", "body"] },
  { label: "right arm", pts: [LX.RSHO, LX.RELB, LX.RWRI],          joints: ["shoulder", "elbow"] },
  { label: "left arm",  pts: [LX.LSHO, LX.LELB, LX.LWRI],          joints: ["shoulder", "elbow"] },
  { label: "right leg", pts: [LX.RHIP, LX.RKNE, LX.RANK],          joints: ["hip", "knee", "ankle"] },
  { label: "left leg",  pts: [LX.LHIP, LX.LKNE, LX.LANK],          joints: ["hip", "knee", "ankle"] },
];
let overlayPulse = 0;

function drawOverlay(lm, zsrc, formOk, jointName) {
  const W = canvas.width, H = canvas.height;
  const jkey = jointName ? String(jointName).split(" ")[0] : "";
  overlayPulse += 0.16;
  ...
  for (const part of BODY_PARTS) {
    ...
    if (n < 2) continue;
    const conf = vsum / part.pts.length;
    const active = jkey && part.joints.includes(jkey);   // izlenen eklem bu parçada mı
    // renk: nötr parça vişne-soluk; izlenen parça form iyi=nane, düzelt=vişne
    const rgb = active ? (formOk ? MINT : CHERRY) : CHERRY;
    const baseA = active ? (formOk ? 0.9 : 0.6 + 0.4 * pulse)   // düzeltme kutusu nabız
                         : 0.32 + 0.4 * Math.min(1, conf);
    const col = (a) => "rgba(" + rgb[0] + "," + rgb[1] + "," + rgb[2] + "," + a + ")";
    const pad = 14;
    minx -= pad; miny -= pad; maxx += pad; maxy += pad;
    const bw = maxx - minx, bh = maxy - miny;

    // köşe-işaretli tespit kutusu (klasik detection köşeleri) — ince
    ctx.strokeStyle = col(baseA);
    ctx.lineWidth = active ? 2 : 1.4;
    const cl = Math.min(22, bw * 0.28, bh * 0.28);
    ...
    ctx.strokeStyle = col(baseA * 0.28);
    ctx.lineWidth = 1;
    ctx.strokeRect(minx, miny, bw, bh);

    // TEK etiket: sadece izlenen parçaya küçük "good/fix" rozeti. parça adları +
    // güven % yok (çakışma + karmaşa yaratıyordu). rozet ayna-GERİ çevrilir ki
    // düz okunsun (canvas scaleX(-1) ayna; metin ters olmasın).
    if (active) {
      const tag = formOk ? "good" : "fix";
      ctx.font = "700 13px 'Courier New', monospace";
      const tw = ctx.measureText(tag).width;
      const bx = minx + bw / 2, by = miny - 4;   // kutu üst-orta
      ctx.save();
      ctx.translate(bx, by);
      ctx.scale(-1, 1);                          // ayna-geri: yazı düz okunur
      ctx.fillStyle = col(baseA);
      ctx.fillRect(-(tw + 12) / 2, -17, tw + 12, 16);
      ctx.fillStyle = "rgba(255,240,246," + Math.min(1, baseA + 0.25) + ")";
