...
## round 4: the jury caught the engine contradicting its own bench (jul 15)
27 findings. two were sharp claim-vs-proof violations, both fixed by OBEYING the measurement:
- world-refine (world-Kalman) SHIPPED ON claiming 'measurably steadier angles', but bench synthcal shows it makes jitter WORSE in bad light (2.14->2.21) - the exact regime it was for. a second Kalman on top of One Euro adds phase lag and drags toward stale velocity in noise. so i turned it OFF by default. the layer stays (for occlusion, and a real golden clip may yet show a win) but it no longer ships degrading the product while the story says it improves it. the measurement refuted the claim; i obeyed the measurement.
- classifier had colliding signatures: situp/glutebridge and jumpingjack/armraise share an identical 4-tuple, so it returned 1.00 confidence on a coin-flip. added tie detection: if 2+ signatures tie for the max, confidence is divided by the tie count, so an ambiguous move gets low confidence and the nudge stays silent instead of being confidently wrong. a test proves a colliding hip move now gets <0.6 confidence.
189 native tests. the honest theme continues: this round the engine was made MORE HONEST and slightly SMALLER (a layer turned off) because its own bench said so.

## calibration hardening: a consistency gate so a moving body can't poison the lock (jul 15)
calibration learned body ratios + bone lengths over 60 frames and took the median - but it had no QUALITY gate. if you fidgeted or the framing wobbled during those 2 seconds, the dirty samples went straight into the median and every downstream identity check was built on a bad measurement. added a consistency gate: after collecting, each ratio/bone must not only have enough samples but also a low coefficient of variation (std/mean <= 0.18 for ratios, 0.15 for bones). a measure that's too scattered (you moved) is marked unusable and left out of the lock. and if fewer than 2 ratios come back reliable (you moved the whole time), calibration doesn't lock on garbage - it re-learns once ('hold still for a moment'). a test proves a jittering body forces a re-learn, then a steady body completes it. the identity lock now rests only on measurements that were actually steady. 191 native tests.
